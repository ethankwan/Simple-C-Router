...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  // FILL THIS IN
	const RoutingTableEntry *found = NULL;
	if(m_entries.empty()){
		found = NULL;
	}
	else{
		std::list<RoutingTableEntry>::const_iterator i = m_entries.begin();
		while(i != m_entries.end()){
		uint32_t masked = ip & i->mask;
		uint32_t goal = i->dest & i->mask;
		if(masked == goal){
			if(found == NULL) //first match
			found = &*i;
			else if(i->mask >= found->mask) //found a better match
			found = &*i;
		}
      i++; //++i
    }
